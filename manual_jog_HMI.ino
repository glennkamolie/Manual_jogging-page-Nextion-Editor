...
float a1X = 0;
float b1X = 0;
float c1X = 0;  // X calibration dots on A4 sheet in Pixy coordinate system
float a1Y = 0;
float b1Y = 0;
float c1Y = 0;  // Y calibration dots on A4 sheet in Pixy coordinate system
float a2X = 0;
float b2X = 0;
float c2X = 0;  // X calibration dots in Dobot coordinate system
float a2Y = 0;
float b2Y = 0;
float c2Y = 0;  // Y calibration dots in Dobot coordinate system
float scaleX = 0;
float scaleY = 0;
float lr1 = 0;
float lr2 = 0;           // length of a-b line segment (used as radius in calculation of angle
...
int tempIntVarSignature = 0;  // variables used for temporarily storing value (used for swapping values of variables)
// other variables
float startPosX = 200.00;
float startPosY = 0.00;
float startPosZ = 0.00;
float startPosR = 0.00;  // Start position when powering up
float calXpos = 200.00;
float calYpos = 0.00;
float calZpos = 100.00;
float calRpos = 0.00;  // Arm position for reading calibration sheet
float dropLocation1X = 200.00;
float dropLocation1Y = 0.00;
float dropLocation1Z = 0.00;
float dropLocation1R = 0.00;  // Drop location for parts with signature 1
float dropLocation2X = 200.00;
float dropLocation2Y = 0.00;
float dropLocation2Z = 0.00;
float dropLocation2R = 0.00;  // Drop location for parts with signature 2
float dropLocation3X = 200.00;
float dropLocation3Y = 0.00;
float dropLocation3Z = 0.00;
float dropLocation3R = 0.00;  // Drop location for parts with signature 3
...
float partWidth = 0;
float partHeight = 0;  // dimensions of part found by pixy camera
float dobotPartX = 0;
float dobotPartY = 0;
...
float intermediateY = 0;  // intermediate variable used in calculations
...
NexDSButton bt0 = NexDSButton(0, 5, "bt0");  // id 5
NexDSButton bt1 = NexDSButton(0, 8, "bt1");
NexDSButton bt2 = NexDSButton(0, 9, "bt2");
NexDSButton bt3 = NexDSButton(0, 10, "bt3");
NexDSButton bt4 = NexDSButton(0, 11, "bt4");
NexDSButton bt5 = NexDSButton(0, 12, "bt5");
NexDSButton bt6 = NexDSButton(0, 13, "bt6");
NexDSButton bt7 = NexDSButton(0, 14, "bt7");
NexDSButton bt8 = NexDSButton(0, 15, "bt8");
...
NexButton b7 = NexButton(0, 6, "b7");
NexButton b12 = NexButton(0, 7, "b12");
...
NexButton b43 = NexButton(2, 16, "b43");
NexText t0 = NexText(0, 1, "t0");
NexText t1 = NexText(0, 3, "t1");
NexText t2 = NexText(0, 16, "t2");
NexText t3 = NexText(0, 17, "t3");
...
NexText t13 = NexText(4, 4, "t13");
NexText t14 = NexText(4, 5, "t14");
NexText t15 = NexText(4, 7, "t15");
...
void page2PushCallback(void *ptr)  // If page 2 is loaded on the display, the following is going to execute:
{
   ...
   if (endeffectorGripper == 1) {
      ...
      Serial2.write(0xff);
      Serial2.write(0xff);
      Serial2.write(0xff);  // set radio button for gripper to on
      Serial2.print("r1.val=");
      Serial2.print(0);
      Serial2.write(0xff);
      Serial2.write(0xff);
      Serial2.write(0xff);  // set radio button for vacuum cup to off
      ...
      Serial2.write(0xff);
      Serial2.write(0xff);
      Serial2.write(0xff);  // set radio button for gripper to off
      Serial2.print("r1.val=");
      Serial2.print(1);
      Serial2.write(0xff);
      Serial2.write(0xff);
      Serial2.write(0xff);  // set radio button for vacuum cup to on
...
void b20PopCallback(void *ptr) {
   ...
   Serial.print(F("Kalibrasi Pos tersimpan di EEPROM:"));
...
void b24PopCallback(void *ptr) {
   ...
   Serial.print(F("Z-Down disimpan di eeprom: Z"));
...
void b26PopCallback(void *ptr) {
   ...
   Serial.print(F("a2X disimpan di eeprom: "));
   Serial.println(a2X);
   EEPROM.get(eeAddressA2y, a2Y);
   Serial.print(F("a2Y disimpan di eeprom: "));
   Serial.println(a2Y);
}
...
void b28PopCallback(void *ptr) {
   ...
   Serial.print(F("b2X disimpan di eeprom: "));
   Serial.println(b2X);
   EEPROM.get(eeAddressB2y, b2Y);
   Serial.print(F("b2Y disimpan di eeprom: "));
   Serial.println(b2Y);
}
...
void b30PopCallback(void *ptr) {
   ...
   Serial.print(F("c2X disimpan di eeprom: "));
   Serial.println(c2X);
   EEPROM.get(eeAddressC2y, c2Y);
   Serial.print(F("c2Y disimpan di eeprom: "));
   Serial.println(c2Y);
}
...
void b38PopCallback(void *ptr) {
   ...
   for (i = 0; i < pixy.ccc.numBlocks; i++) {
      if (pixy.ccc.blocks[i].m_signature == signatureCalib) {
         CalibDotFound++;
      }                            // check if signature is a calibration dot and add to counter if true
   ...
   if (CalibDotFound == 3) {
      ...
      for (j = 0; j < pixy.ccc.numBlocks; j++) {
         ...
         if (pixy.ccc.blocks[j].m_signature == signatureCalib) {
            ...
            }  // flip Y coordinate from Pixy by subtracting it from the max number of Y pixels on camera (207)
      ...
      Serial.print(F("a1X disimpan di eeprom: "));
      Serial.println(a1X);
      EEPROM.get(eeAddressA1y, a1Y);
      Serial.print(F("a1Y disimpan di eeprom: "));
      Serial.println(a1Y);
      ...
      Serial.print(F("b1X disimpan di eeprom: "));
      Serial.println(b1X);
      EEPROM.get(eeAddressB1y, b1Y);
      Serial.print(F("b1Y disimpan di eeprom: "));
      Serial.println(b1Y);
      ...
      Serial.print(F("c1X disimpan di eeprom: "));
      Serial.println(c1X);
      EEPROM.get(eeAddressC1y, c1Y);
      Serial.print(F("c1Y disimpan di eeprom: "));
      Serial.println(c1Y);

   } else {
      Serial.println(" ");
      Serial.print("Error: incorrect numer of blocks found (3 required, found: ");
...
void b42PopCallback(void *ptr) {
   ...
   Serial.print(F("speedRatio disimpan di eeprom: "));
   ...
   Serial.print(F("accelRatio disimpan di eeprom: "));
...
void r0PushCallback(void *ptr)  // Press event for radio checkbox
{
   ...
   Serial.print("End effector type disimpan di eeprom: ");
...
void setup() {
   ...
   if (eePromIntVar != 999) {
      ...
      startPosZ = 0.00;
      startPosR = 0.00;
      Serial.println(F("Start position NOT previously saved, using default values:"));
      Serial.print(F(" X"));
   ...
   if (eePromIntVar != 999) {
      ...
      calZpos = 160.00;
      calRpos = 0.00;
      Serial.println("Calibration position NOT previously saved, using default values:");
      Serial.print(F(" X"));
   ...
   if (eePromIntVar != 999) {
      a1X = 0.00;
      a1Y = 0.00;
      Serial.println(F("Posisi titik kalibrasi A1 TIDAK disimpan sebelumnya, using default values:"));
      Serial.print(F(" X"));
      Serial.print(a1X);
      Serial.print(F(" Y"));
      Serial.print(a1Y);
   ...
   if (eePromIntVar != 999) {
      b1X = 0.00;
      b1Y = 0.00;
      Serial.println(F("Posisi titik kalibrasi B1 TIDAK disimpan sebelumnya, using default values:"));
      Serial.print(F(" X"));
      Serial.print(b1X);
      Serial.print(F(" Y"));
      Serial.print(b1Y);
   ...
   if (eePromIntVar != 999) {
      c1X = 0.00;
      c1Y = 0.00;
      Serial.println(F("Posisi titik kalibrasi C1 TIDAK disimpan sebelumnya, using default values:"));
      Serial.print(F(" X"));
      Serial.print(c1X);
      Serial.print(F(" Y"));
      Serial.print(c1Y);
   ...
   if (eePromIntVar != 999) {
      a2X = 200.00;
      a2Y = -50;
      Serial.println(F("Posisi titik kalibrasi A2 TIDAK disimpan sebelumnya, using default values:"));
      Serial.print(F(" X"));
      Serial.print(a2X);
      Serial.print(F(" Y"));
      Serial.print(a2Y);
   ...
   if (eePromIntVar != 999) {
      b2X = 200.00;
      b2Y = 50;
      Serial.println("Posisi titik kalibrasi B2 TIDAK disimpan sebelumnya, using default values:");
      Serial.print(" X");
      Serial.print(b2X);
      Serial.print(" Y");
      Serial.print(b2Y);
   ...
   if (eePromIntVar != 999) {
      c2X = 250.00;
      c2Y = -50;
      Serial.println("Posisi titik kalibrasi C2 TIDAK disimpan sebelumnya, using default values:");
      Serial.print(" X");
      Serial.print(c2X);
      Serial.print(" Y");
      Serial.print(c2Y);
   ...
   if (eePromIntVar != 999) {
      zDownPos = 0.00;
      Serial.print(F("Posisi titik kalibrasi Z-Down TIDAK disimpan sebelumnya, using default values:"));
      Serial.print(F(" Z"));
      Serial.println(zDownPos);
   }
   if (eePromIntVar == 999) {
   ...
   if (eePromIntVar != 999) {
      ...
      dropLocation1R = 0.00;
      Serial.println(F("Drop position 1 NOT previously saved, using default values:"));
      Serial.print(F(" X"));
   ...
   if (eePromIntVar != 999) {
      ...
      dropLocation2Z = 0.00;
      dropLocation2R = 0.00;
      Serial.println(F("Drop position 2 NOT previously saved, using default values:"));
      Serial.print(F(" X"));
   ...
   if (eePromIntVar != 999) {
      ...
      dropLocation3Z = 0.00;
      dropLocation3R = 0.00;
      Serial.println(F("Drop position 3 NOT previously saved, using default values:"));
      Serial.print(F(" X"));
   ...
   if (eePromIntVar != 999) {
      endeffectorGripper = 0;
      Serial.println(F("End effector type NOT previously saved, using default: Vacuum Cup"));
   }
   if (eePromIntVar == 999) {
      EEPROM.get(eeAddressEndeffector, endeffectorGripper);
   ...
   if (eePromIntVar != 999) {
      speedRatio = 50;
      Serial.print(F("speedRatio NOT previously saved, using default value: "));
      Serial.println(speedRatio);
   }
   if (eePromIntVar == 999) {
   ...
   if (eePromIntVar != 999) {
      accelRatio = 50;
      Serial.print(F("accelRatio NOT previously saved, using default value: "));
      Serial.println(accelRatio);
   }
   if (eePromIntVar == 999) {
   ...
   if (eePromIntVar != 999) {
      delayInterval = 1500;
      Serial.print(F("delayInterval NOT previously saved, using default value: "));
      Serial.println(delayInterval);
   }
   if (eePromIntVar == 999) {
            ...
            case 3:
               ...
               for (i = 0; i < pixy.ccc.numBlocks; i++) {
                  if (pixy.ccc.blocks[i].m_signature == signaturePart1) {
                     signature1Found++;
                     j = i;
                     activeSignature = 1;  // check if signature is a part 1 signature and set j counter to array address if true;
                     pixy.ccc.blocks[i].print();
                  }  // print found signature on serial monitor
                  else if (pixy.ccc.blocks[i].m_signature == signaturePart2) {
                     signature2Found++;
                     j = i;
                     activeSignature = 2;  // check if signature is a part 1 signature and set j counter to array address if true;
                     pixy.ccc.blocks[i].print();
                  }  // print found signature on serial monitor
                  else if (pixy.ccc.blocks[i].m_signature == signaturePart3) {
                     signature3Found++;
                     j = i;
                     activeSignature = 3;  // check if signature is a part 1 signature and set j counter to array address if true;
                     pixy.ccc.blocks[i].print();
                  }  // print found signature on serial monitor
               }
               if (signature1Found > 0 || signature2Found > 0 || signature3Found > 0) {
                  ...
                  Serial.print("pixyPartY: after flipping Y axis with 207 - pixyPartY: ");
                  Serial.println(pixyPartY);
                  ...
                  Serial.println(lr2);
                  // translate to origin
                  dobotPartX = pixyPartX - a1X;
                  Serial.print("dobotPartX after translate to origin: ");
                  Serial.println(dobotPartX);  // translate to origin
                  dobotPartY = pixyPartY - a1Y;
                  Serial.print("dobotPartY after translate to origin: ");
                  Serial.println(dobotPartY);  // translate to origin
                  angle1 = (acos((b1X - a1X) / lr1));
                  Serial.print("angle1: ");
                  Serial.println(angle1);  // angle between calibration sheet and pixy coordinate system
                  dobotPartX = (dobotPartX * cos(-1 * angle1)) + (dobotPartY * sin(-1 * angle1));
                  Serial.print("dobotPartX after rotate: ");
                  Serial.println(dobotPartX);  // rotate X coordinate from sheet to Pixy coordinate system
                  dobotPartY = ((-1 * dobotPartX) * sin(-1 * angle1)) + (dobotPartY * cos(-1 * angle1));
                  Serial.print("dobotPartY after rotate: ");
                  Serial.println(dobotPartY);  // rotate Y coordinate from sheet to Pixy coordinate system
                  dobotPartX = dobotPartX * scaleX;
                  Serial.print("dobotPartX after scale: ");
                  Serial.println(dobotPartX);  // scale X between Pixy and Dobot
                  dobotPartY = dobotPartY * scaleY;
                  Serial.print("dobotPartY after scale: ");
                  Serial.println(dobotPartY);  // scale Y between Pixy and Dobot
                  angle2 = (acos((b2X - a2X) / lr2));
                  Serial.print("angle2: ");
                  Serial.println(angle2);  // angle between Pixy and Dobot coordinate systems
                  intermediateX = dobotPartX;
                  intermediateY = dobotPartY;  // write to intermediate variable to avoid miscalculation in next step
                  dobotPartX = (intermediateX * cos(angle2)) + (intermediateY * sin(angle2));
                  Serial.print("dobotPartX after rotate to Dobot coordinate system: ");
                  Serial.println(dobotPartX);  // rotate X coordinate to Dobot coordinate system
                  dobotPartY = ((-1 * intermediateX) * sin(angle2)) + (intermediateY * cos(angle2));
                  Serial.print("dobotPartY dobotPartX after rotate to Dobot coordinate system: ");
                  Serial.println(dobotPartY);  // rotate Y coordinate to Dobot coordinate system
                  dobotPartX = dobotPartX + a2X;
                  Serial.print("dobotPartX after translate: ");
                  Serial.println(dobotPartX);  // translate from origin to destination
                  dobotPartY = dobotPartY + a2Y;
                  Serial.print("dobotPartY after translate: ");
                  Serial.println(dobotPartY);  // translate from origin to destination
                  ...
                  moveStep = 0;
                  Serial.println(" ");
                  Serial.print("Error: Tidak ditemukan object, at least 1 part required to start pick and place action!!!");
